...
if (isset($_POST['submit'])) {
    ...
    if (empty($sno)) {
        ...
        $sql = "SELECT * FROM `liabrary` WHERE `s_no` = $sno";
        $result = $conn->query($sql);

        if ($result->num_rows == 0) {

            $sql = "INSERT INTO `liabrary`(`s_no`, `name`, `author`, `price`, `quantity`, `publish`, `publisher`) VALUES ('$sno','$name','$author','$price','$quantity','$publish','$publisher')";
            $result = $conn->query($sql);
...
<body class="bg-warning font-monospace">
    <div class="container">
        <div class="row">
            <h3 class="text-center mt-5 text-primary font-monospace">Enter new book here !</h3>
            <div>
